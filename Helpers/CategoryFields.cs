...
public static class CategoryFields
{
    ...
    /// ИСПРАВЛЕНИЕ: используем System.Text.Json вместо самописного сериализатора
    /// (самописный не экранировал кавычки, спецсимволы и Unicode-символы).
    /// </summary>
    public static string ToJson()
    {
        // Преобразуем в словарь, где ключ — строка (CategoryID)
        var dict = _byCategory.ToDictionary(
            kv => kv.Key.ToString(),
