...
public class DetailModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostSendMessageAsync(int id)
    {
        ...
        if (string.IsNullOrWhiteSpace(MessageText)) { TempData["Error"] = "Введите текст сообщения"; return RedirectToPage(new { id }); }

        Ad = await _db.Advertisements.FindAsync(id);
        if (Ad == null) return NotFound();
        if (Ad.UserID == uid) { TempData["Error"] = "Нельзя отправить сообщение самому себе"; return RedirectToPage(new { id }); }

        _db.Messages.Add(new Message
        {
            SenderID = uid.Value, ReceiverID = Ad.UserID, AdID = id,
            Text = MessageText, SentDate = DateTime.UtcNow
        });
